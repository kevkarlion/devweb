...
// Local fallback for LustreText to avoid a missing-module/type error.
// This lightweight implementation renders text with a clipped gradient.
// If you have a project-level LustreText component, remove this and restore the original import.
function LustreText({ text }: { text: string }) {
  return (
    <span
      aria-hidden={false}
      className="inline-block bg-gradient-to-r from-sky-400 via-cyan-400 to-emerald-400 bg-clip-text text-transparent"
    >
      {text}
    </span>
  );
...
function AnnouncementComponent({
  ...
  function isExpandableContent(
  child: React.ReactNode
): child is React.ReactElement<{ children: React.ReactNode }> {
  return (
    React.isValidElement(child) &&
    (child.type as unknown as Record<symbol, boolean>)?.[EXPANDABLE_CONTENT_SYMBOL] === true
  );
}
...
useEffect(() => {
  ...
  childArray.forEach((child) => {
    if (isExpandableContent(child)) {
      expanded = child.props.children; // ✅ TypeScript sabe que tiene children
      found = true;
    } else {
      main.push(child);
    }
